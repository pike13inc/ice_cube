  ...
  class Rule
    ...
    def self.from_ical ical
      ...
      ical.split(';').each do |rule|
        ...
        when 'UNTIL'
          until_time = DateTime.parse(value).to_time.utc
          date_only =
            begin
              until_time == Date.parse(value)
            rescue
              false
            end
          if date_only # no time specified, the date must be inclusive
            until_time = TimeUtil.end_of_date(until_time).to_time.utc
          end
